...
class RestaurantOrchestrator:
    ...
    def _create_agent_tools(self) -> List[FunctionTool]:
        ...
        for agent_id, agent in self.specialist_agents.items():
            ...
            # Crear función transfer_to_agent para cada especialista

        def create_transfer_function(agent_instance, config_instance):
            async def transfer_func(query: str) -> str:
                """Transfiere consulta al especialista"""
                return f"Transfiriendo '{query}' a {config_instance.name}"

            transfer_func.__name__ = (
                f"consult_{config_instance.specialization}_specialist"
            )
            return transfer_func

        transfer_func = create_transfer_function(agent, config)
        agent_tools.append(FunctionTool(transfer_func))
    ...
    def _create_orchestrator_agent(self) -> LlmAgent:
        ...
        def analyze_user_intent(query: str, context: str = "") -> Dict:
            ...
            confidence = scores.get(best_agent, 0) / len(
                AGENT_CONFIGS[best_agent].keywords
            )
            ...
            return {
                ...
                < self.routing_config["confidence_threshold"],
            }

        # Agregar funciones como herramientas
        function_tools = [
            FunctionTool(self.detect_language),
            FunctionTool(self.analyze_user_intent),
        ]
...
""",
    ...
    async def process_customer_query(
        self, query: str, session_context: Dict = None
    ) -> str:
        """
    ...
    async def _consult_specialist(
        self, agent_id: str, query: str, context: Dict
    ) -> str:
    ...
    def _unify_response(
        self, original_query: str, specialist_response: str, context: Dict
    ) -> str:
...
"""
    ...
    def analyze_user_intent(self, query: str, context: str = "") -> Dict:
        ...
        best_agent = max(scores, key=scores.get) if scores else "food_agent"
        confidence = (
            scores.get(best_agent, 0) / len(AGENT_CONFIGS[best_agent].keywords)
            if AGENT_CONFIGS[best_agent].keywords
            else 0
        )
