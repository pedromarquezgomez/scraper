...
from .config.system_config import SystemConfig, SYSTEM_MESSAGES
...
class RestaurantMultiAgentSystem:
    ...
    def __init__(self):
        ...
        self.runner = InMemoryRunner(
            agent=self.orchestrator.orchestrator_agent,
            app_name=self.config.app_name,
            session_service=self.session_service
        )
        
        print("🍽️ Sistema Multiagente del Restaurante inicializado correctamente!")
        print(f"📋 Agentes especializados activos: {list(self.specialist_agents.keys())}")
    ...
    async def process_query(self, query: str, session_id: str = None, user_id: str = "default_user") -> str:
        """
    ...
    def _show_agents(self):
        ...
        for agent_id, config in AGENT_CONFIGS.items():
            status = "✅ Activo" if agent_id in self.specialist_agents else "⏳ Pendiente"
            print(f"\n{status} {config.name}")
